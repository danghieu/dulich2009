...
function ajaxHandleResponse(div_output, waiting_type, output_type){
	...
	if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200)){
		if (output_type == 1){
			...
			document.getElementById(div_output).length = 0;
			document.getElementById(div_output).options[0] = new Option("--Select--","0");
			
			var	response = xmlhttp.responseXML.documentElement;
			var _data = response.getElementsByTagName('category');
			if(_data.length == 0)
			{
				document.getElementById(div_output).options[0] = new Option("No city available",'0');	
			}
			var i
			for ( i = 0 ; i < _data.length ; i ++ )
			{
				document.getElementById(div_output).options[i+1] = new Option(response.getElementsByTagName('fname')[i].firstChild.data,response.getElementsByTagName('id')[i].firstChild.data);	
			}	
		}	
		else{
			//Display result
