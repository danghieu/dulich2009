...
public class OntologyModel {
	...
	public String prefix = "PREFIX chiase: <http://www.owl-ontologies.com/Ontology1254153498.owl#> \n";
	//public String namespace = "http://localhost:8080/Ontology/Message/Message.owl#";
	public String namespace = "http://www.owl-ontologies.com/Ontology1254153498.owl#";
	public OntModel om;
	public String path = "C:/Program Files/Apache Software Foundation/Tomcat 6.0/webapps/MyOntology/Message/Message_simple.owl";
...
// cho nay dua ra cac the hien cha con// kho	
private void builtClass(OntModel om) {
	...
	public String[] searchbyPrefix(String prefix) {
		...
		String queryString = "PREFIX chiase: <" + Message.NS + "> \n"
				+ "SELECT ?tukhoa \n" + "WHERE \n" + "{ \n"
...
/*
 * Tim kiem de dua ra 1 mang doi tuog theo tu khoa nao day
 * 
 * 
	...
	*/
	...
	public long maxIdPlus(String oClass) {
		...
		Database.LoadOnt2Database();
		OntModel om = Database.getOntologyModel();
		String strQuery = "" + "PREFIX chiase: <" + Message.NS + "> \n"
				+ "PREFIX rdf: <" + Message.RDF + "> \n" + "PREFIX rdfs: <"
				+ Message.RDFS + "> \n" + "SELECT ?x \n" + "WHERE \n" + "{ \n"
				+ " ?x rdf:type <" + Message.NS + oClass + "> \n" + "} \n" + "";
		Query query = QueryFactory.create(strQuery);
		QueryExecution queryexec = QueryExecutionFactory.create(query, om);
		try {
			ResultSet rs = queryexec.execSelect();
	...
	public static String KetQuaDaLuu(String tukhoa, String tailieuid) {
		...
		String strQuery = "" + "PREFIX chiase: <" + Message.NS + "> \n"
				...
				+ " ?x chiase:KetQuaLaTaiLieu <" + Message.NS + tailieuid
	...
	/*	
		try {
			...
			// chua co lop nay thi tao ra 1 lop moi
				...
				OntClass oc = om.createClass(Message.getURI() + OntClass);
				int total = list.size() + 1;
				Individual ind = om.createIndividual(Message.getURI()
						+ OntClass + "_" + total, oc);
				
				// Ten lop, chu y tat ca deu phai co namespace
				if (TenGoi != null) {
					ind.addProperty(Message.TenGoi, TenGoi);
				}
	...
	public static Individual GetbyInstance(String Instance) {
		Database.LoadOnt2Database();
		OntModel om = Database.getOntologyModel();
		Individual ind = om.getIndividual(Message.getURI() + Instance);
		return ind;
	}
	...
	public static void removePropertyData(String Ind, Property pro) {
		Database.LoadOnt2Database();
		OntModel om = Database.getOntologyModel();
		Individual ind = om.getIndividual(Message.getURI() + Ind);
		ind.removeAll(pro);
		om.commit();
	}
